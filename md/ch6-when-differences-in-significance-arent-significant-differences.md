# When differences in significance aren’t significant differences<a
href="#when-differences-in-significance-aren-t-significant-differences"
class="headerlink" title="Permalink to this headline">¶</a>

“We compared treatments A and B with a placebo. Treatment A showed a
significant benefit over placebo, while treatment B had no statistically
significant benefit. Therefore, treatment A is better than treatment B.”
...
class="reference internal">22</a></sup></span>

One reason is the arbitrary nature of the <span class="math">\\p &lt;
...
second reason is that *p* values are not measures of effect size, so
similar *p* values do not always mean similar effects. Two results with
...
<span id="confidence-intervals"></span><span id="index-0"></span>

## When significant differences are missed<a href="#when-significant-differences-are-missed" class="headerlink"
...
![Two group means plotted with overlapping confidence
intervals](_images/confidence.png)
...
1.  The standard deviation of the measurements. Calculate how far each
...
2.  The standard error of some estimator. For example, perhaps the error
    ...
    different samples of patients, each containing exactly *n* subjects,
...
intervals should include the “true” value.

In the example plot, we have two 95% confidence intervals which overlap.
...
*might not* be different – the average time to recover could be 25 in
...
class="std std-ref">p value</span></a> be?

In this case, <span class="math">\\p&lt; 0.05\\</span>. There is a
statistically significant difference between the groups, even though the
confidence intervals
overlap.<a href="#ttest" id="id1" class="footnote-reference">[1]</a>
...
measurements are not significantly different even when they are.
...
A survey of psychologists, neuroscientists and medical researchers found
...
confidence intervals which *can* be compared by eye, and even correct
...
class="reference internal">19</a></sup></span>
...
mean two values *are* significantly different. It’s always best to use
the appropriate hypothesis test instead. Your eyeball is not a
well-defined statistical procedure.

<table id="ttest" class="docutils footnote" data-frame="void"
data-rules="none">
