# An introduction to data analysis<a href="#an-introduction-to-data-analysis" class="headerlink"
...
<span id="p-values"></span><span id="index-0"></span>

## The power of *p* values<a href="#the-power-of-p-values" class="headerlink"
title="Permalink to this headline">¶</a>

Statistics provides the answer. If we know the *distribution* of typical
...
ineffective, what are the chances I’d see data like what I saw?”

That’s a bit tricky, so read it again.

Intuitively, we can see how this might work. If I only test the
...
unlikely that *all* of them will have shorter colds than average,
*unless my medication works.*
...
> class="reference internal">24</a></sup></span>

So if I give my medication to 100 patients and find that their colds are
a day shorter on average, the *p* value of this result is the chance
that, if my medication didn’t do anything at all, my 100 patients would
randomly have, on average, day-or-more-shorter colds. Obviously, the *p*
...
test the medication on.

That’s a tricky concept to wrap your head around. A *p* value is not a
measure of how right you are, or how significant the difference is; it’s
a measure of *how surprised you should be* if there is no actual
...
It’s not easy to translate that into an answer to the question “is there
really a difference?” Most scientists use a simple rule of thumb: if *p*
...
between medication and placebo “significant.” If *p* is larger, we’ll
call the difference insignificant.

But there are limitations. The *p* value is a measure of surprise, not a
measure of the size of the effect. I can get a tiny *p* value by either
...
Similarly, statistical *in*significance is hard to interpret. I could
...
insignificant difference does not mean there is no difference at all.
