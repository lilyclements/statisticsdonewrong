...
# The *p* value and the base rate fallacy<a href="#the-p-value-and-the-base-rate-fallacy" class="headerlink"
...
![](_images/drug-grids-2.png)

Of the ninety ineffectual drugs, I will conclude that about 5 have
significant effects. Why? Remember that *p* values are calculated under
the assumption of no effect, so <span class="math">\\p = 0.05\\</span>
means a 5% chance of falsely concluding that an ineffectual drug works.
...
![](_images/drug-grids-3.png)

The chance of any given “working” drug being truly effectual is only
...
which are really false positives – is 38%.

Because the *base rate* of effective cancer drugs is so low – only 10%
...
rate fallacy*. Remember how *p* values are defined:
...
A *p* value is calculated under the assumption that the medication *does
not work* and tells us the probability of obtaining the data we did, or
data more extreme than it. It does *not* tell us the chance the
medication is effective.

When someone uses their *p* values to say they’re probably right,
...
likely that *most* “statistically significant” results with
<span class="math">\\p &lt; 0.05\\</span> are actually flukes.

One good example is medical diagnostic tests.

<span id="index-2"></span>

## The base rate fallacy in medical testing<a href="#the-base-rate-fallacy-in-medical-testing" class="headerlink"
title="Permalink to this headline">¶</a>
...
chances you have breast cancer?

Ignoring the chance that you, the reader, are
...
their mammograms, giving us 70 women incorrectly told they have cancer.
...
same error. *P* values are counterintuitive, and the base rate fallacy
is everywhere.

<span id="base-rate-gun"></span><span id="index-3"></span>

## Taking up arms against the base rate fallacy<a href="#taking-up-arms-against-the-base-rate-fallacy"
...
class="reference internal">28</a></sup></span>
...
question, then you’ll end up *vastly* overestimating the use of guns in
self-defense.
...
This is exactly analogous to the cancer drug example earlier. Here, *p*
is the probability that someone will falsely claim they’ve used a gun in
self-defense. Even if *p* is small, your final answer will be wildly
wrong.

To lower *p*, criminologists make use of more detailed surveys. The
...
a much smaller chance of massive overestimation.

<span id="multiple-comparisons"></span><span id="index-4"></span>

## If at first you don’t succeed, try, try again<a href="#if-at-first-you-don-t-succeed-try-try-again"
class="headerlink" title="Permalink to this headline">¶</a>

The base rate fallacy shows us that false positives are much more likely
...
<span class="caption-text">Cartoon from xkcd, by Randall Munroe.
<a href="http://xkcd.com/882/"
class="reference external">http://xkcd.com/882/</a></span>

As you can see, making multiple comparisons means multiple chances for a
...
the chance of false positive is as high as 90%.
...
measure, and you’ll find that *something* causes cancer. Ask enough
...
class="reference internal">54</a></sup></span>

There exist techniques to correct for multiple comparisons. For example,
...
papers even consider it.

<span id="red-herrings"></span><span id="index-5"></span>

## Red herrings in brain imaging<a href="#red-herrings-in-brain-imaging" class="headerlink"
title="Permalink to this headline">¶</a>
...
One study, for instance, tested the effects of an “open-ended
...
class="reference internal">8</a></sup></span>
...
class="reference internal">13</a></sup></span>

<span id="false-discovery"></span><span id="index-6"></span>

## Controlling the false discovery rate<a href="#controlling-the-false-discovery-rate" class="headerlink"
title="Permalink to this headline">¶</a>
...
This has drawbacks. By lowering the *p* threshold required to declare a
...
improve the statistical power, but they are not magic solutions.

Worse, they don’t spare you from the base rate fallacy. You can still be
misled by your *p* threshold and falsely claim there’s “only a 5% chance
...
statistical test that will let me control this fraction?

For many years the answer was simply “no.” As you saw in the section on
...
but we’d rather find that out from the data, rather than guessing.

In 1995, Benjamini and Hochberg provided a better answer. They devised
an exceptionally simple procedure which tells you which *p* values to
...
1.  Perform your statistical tests and get the *p* value for each. Make
    a list and sort it in ascending order.
2.  Choose a false-discovery rate and call it *q*. Call the number of
    statistical tests *m*.
3.  Find the largest *p* value such that <span class="math">\\p \leq i
    q/m\\</span>, where *i* is the *p* value’s place in the sorted list.
4.  Call that *p* value and all smaller than it statistically
    significant.

You’re done! The procedure guarantees that out of all statistically
significant results, no more than *q* percent will be false
positives.<span class="citation"><sup><a href="zbibliography.html#citation-benjamini-1995ws"
class="reference internal">7</a></sup></span>

The Benjamini-Hochberg procedure is fast and effective, and it has been
...
better statistical power when testing certain kinds of data.
...
<table id="male" class="docutils footnote" data-frame="void"
data-rules="none">
